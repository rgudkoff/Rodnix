...
| CT-005 | LIFECYCLE | `exit` переводит в lifecycle ожидания (`ZOMBIE`/collect) | contract mode в `userland/init/init.c` | PLANNED |
| CT-006 | LIFECYCLE | `waitpid` собирает статус и завершает lifecycle | contract mode: второй `waitpid` по тому же PID -> ошибка | PLANNED |
...
3. Для `CT-003/CT-005/CT-006/CT-011` добавлены заготовки contract probes
   (`contract_exec_probe/after`, расширенный contract mode в `init`), но
   перевод в AUTO отложен до стабилизации wait/lifecycle.
4. `/bin/contract_spawn_wait` и `/bin/contract_fd` доступны как отдельные
   userland-контракты для ручного/аддитивного запуска.
5. `CT-009` частично покрыт обычным boot/userland smoke.

## 5. Минимальный план автоматизации (следующий шаг)

1. Довести `CT-005/CT-006` до AUTO после стабилизации wait/reaper lifecycle.
2. Перевести `CT-003/CT-011` в AUTO после стабилизации exec+wait связки.
3. Автоматизировать `CT-004` (ожидание не-дочернего PID).
4. Добавить `CT-012` (cooperative progress через `SYS_NOP` в polling-loop).
5. Перевести `readdir` на handle-based модель и добавить контракт для нового API.
6. Добавить parser/summary отчёт по `[CT]` маркерам.

## 5.1 Критерий разморозки CT-005 (из PLANNED -> AUTO)

`CT-005` переводится в `AUTO`, когда одновременно выполнены условия:
