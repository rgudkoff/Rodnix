...
- Практическая POSIX-совместимость как основной контракт для userland ABI,
  shell-поведения и базовых утилит.
...
- Userspace разделён на 2 программы:
  ...
  - языковая политика для userland:
    - базовый runtime (`/bin/init`, `/bin/sh`, `crt0`, `libc-lite`, ABI-boundary)
      остаётся в первую очередь C-ориентированным;
    - внешние утилиты допускаются как на C, так и на Rust;
    - Rust рассматривается как нормальный путь для новых standalone-утилит,
      если он не усложняет bootstrap и не добавляет лишнюю runtime-связность;
    - Python не используется для on-target userland и остаётся host-side tooling.
  - В rootfs добавлены внешние заготовки утилит `/bin/ls`, `/bin/cat`, `/bin/true`.
  - Проверен первый Rust userland-path:
    - `/bin/rust_hello` — минимальный smoke/demo бинарник;
    - `/bin/rust_echo` — первая простая внешняя утилита на Rust.

## Контракт совместимости

- RodNIX ориентируется не на "Unix-подобность в целом", а на максимально
  практическую POSIX-совместимость userland.
- Собственные расширения допустимы только как аддитивные возможности:
  они не должны менять ожидаемую семантику POSIX-пути.
- `spawn`, Fabric-специфичные syscalls и прочие RodNIX-specific интерфейсы
  допускаются как расширения, но не как замена POSIX baseline.
