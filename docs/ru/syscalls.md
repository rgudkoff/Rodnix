...
- Целью слоя является практическая POSIX-совместимость по семантике, а не
  только наличие похожих имён syscall-ов.
...
- Если syscall реализует POSIX-путь, его поведение должно стремиться к
  ожидаемой POSIX-семантике по return codes, `errno`, lifetime и
  inheritance rules.
- RodNIX-specific syscalls допускаются как расширения, но не должны
  подменять или искажать базовый POSIX-контракт.
