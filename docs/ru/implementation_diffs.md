...
Разница:
- Нет тонкой политики повышения/понижения приоритета.
- Нет динамического перераспределения между очередями.
...
- Нет передачи портов.
- Нет блокирующего ожидания/таймаутов.
- Нет IDL/генерации.
...
- Реализован только `SYS_NOP`.

Разница:
- BSD‑слой отсутствует.
...
Реализация:
- Есть каркас `net_init()` без функциональности.

Разница:
- Нет сетевого стека, сокетов, loopback, UDP/TCP и драйверов сетевых устройств.
