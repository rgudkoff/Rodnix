...
### Этап 0: Подготовка (текущая ветка `fs`)

- [x] Создать ветку для порта.
- [x] Подтянуть в Rodnix нужные ext2fs-заголовки FreeBSD в отдельный namespace
      (только структуры/константы, без vnodes).
- [x] Добавить тестовый ext2-образ для сценариев роста файла и truncate.
...
- [x] Гарантировать порядок записи метаданных (минимум consistency без журнала).
- [x] Добавить проверки ошибок I/O и best-effort rollback там, где возможно.
...
- `scripts/ci/smoke_qemu.sh` и `scripts/ci/contract_qemu.sh` проходят (`ALL PASS`).
