# Пересмотр структуры файлов ядра (доменный подход)

Этот документ фиксирует целевую структуру каталогов RodNIX для дальнейшей
разработки. Основа идеи: группировка кода по доменам ядра
(`bsd/kern` как доменный слой), а не как "общая папка для всего".

Референс:
- `docs/ru/archive/kernel_crosswalk.md`

## Что видно в доменном подходе
