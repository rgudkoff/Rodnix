...
Документы в `docs/ru/` должны оставаться короткими, операционными и
согласованными с текущим деревом исходников.

Текущая релизная baseline-версия: `0.1.22`.

## Целевая аудитория

RodNIX делается для креаторов — музыка, видео и смежный real-time медиаконтент.
Отсюда главный инженерный критерий: детерминированная латентность важнее
средней пропускной способности. Подробнее — в `overview.md`.
...
5. `vfs.md` / `userspace.md` / `syscalls.md` — прикладные спецификации
   подсистем.
6. `../../README.md` — верхнеуровневые заметки о сборке, релизе и runtime-потоке.
...
- `framebuffer_ownership_spec.md` — контракт владения экраном и `/dev/fb0`.
...
- `contributing_workflow.md` — веточная политика и практический workflow изменений.
...
- `lock_order.md` — верификатор порядка блокировок и наш порядок, записанный впервые.
- `latency_hygiene.md` — границы на время недоступности процессора и что они нашли.
- `time.md` — источник времени, цепочка доверия и четыре ошибки, которые она вскрыла.
- `xnu_processor_model.md` — как XNU устроил поток и процессор, и почему наша гонка неизбежна.
- `sched_locking.md` — протокол block/wake/switch у XNU и FreeBSD, наши разрывы и арбитр.
- `mm_architecture.md` — разбор управления памятью у FreeBSD/XNU и граница pmap.
- `mm_redesign.md` — проект переписывания памяти: классы обслуживания, возврат, учёт.
...
- документы с аудитами, ревью и разовыми планами допустимо хранить как снимки,
  но их не следует использовать как активную архитектурную спецификацию.
