...
32-битное слово, в котором дескриптор со значением 32 и выше непредставим.
Расширение — смена пользовательского ABI, вне области этой спеки. `poll`
ограничений не имеет и является предпочтительным интерфейсом.
...
Контрактные проверки живут в `run_contract_mode_if_enabled()`
(`userland/init/init.c`) и логируются через `ct_log(id, verdict, msg)` — это
существующий механизм набора, отдельных бинарников для них не заводится.
Занятые идентификаторы — до `CT-033`; новые:

| id | проверяет |
|---|---|
| `CT-034` | `dup` + запись в оба дескриптора — offset общий |
| `CT-035` | `dup(socket)` работает; закрытие одного не рвёт второй |
| `CT-036` | чтение unlink'нутого файла до последнего `close` |
| `CT-037` | сокет проходит через ops, а не через отдельный путь |
| `CT-038` | `O_NONBLOCK` разделяется через `dup`, `FD_CLOEXEC` — нет |
| `CT-039` | `poll` с таймаутом не даёт роста CPU-времени задачи |
| `CT-040` | таблица дескрипторов растёт за пределы 32 |
| `CT-041` | `fork` разделяет описание; `close` у родителя не рвёт fd потомка |

`CT-039` читает накопленные тики CPU из `/proc/self/stat` — последнее поле
строки, уже отдаваемое `fs/procfs.c:357`, менять procfs не нужно. Тик равен
10 мс (`SCHEDULER_TIME_SLICE_MS`), так что busy-wait на 300 мс даёт порядка 30
тиков, а нормальный сон — ноль или единицы.
...
- Расширение пространства ошибок (`include/error.h`, сейчас 10 кодов) до набора,
  нужного сети
- Перевод `read`/`write` на copyin/copyout пользовательских буферов; в этой
  спеке на копирование в ядро переводится только массив `pollfd`
