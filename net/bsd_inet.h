...
typedef struct bsd_tcphdr {
    uint16_t th_sport;   /* source port, network order */
    uint16_t th_dport;   /* dest port, network order */
    uint32_t th_seq;     /* sequence number, network order */
    uint32_t th_ack;     /* acknowledgment number, network order */
    uint8_t  th_off;     /* data offset (hdr len / 4) in upper 4 bits */
    uint8_t  th_flags;
    uint16_t th_win;     /* window size, network order */
    uint16_t th_sum;     /* checksum, network order */
    uint16_t th_urp;     /* urgent pointer, network order */
} __attribute__((packed)) bsd_tcphdr_t;

#define BSD_TH_FIN  0x01u
#define BSD_TH_SYN  0x02u
#define BSD_TH_RST  0x04u
#define BSD_TH_PSH  0x08u
#define BSD_TH_ACK  0x10u
...
uint16_t bsd_tcp4_checksum(uint32_t src_host, uint32_t dst_host, const bsd_tcphdr_t* th, const void* payload, size_t payload_len);
