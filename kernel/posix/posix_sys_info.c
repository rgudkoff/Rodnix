...
#include "../fabric/driver/driver.h"
                             ...
                             uint64_t a6)
{
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (dev->attached_driver_name) {
            strncpy(info.driver,
                    dev->attached_driver_name,
        ...
        info.attached = (dev->dispatch_state == FABRIC_DEV_DISPATCH_ATTACHED) ? 1u : 0u;
        info.dispatch_state = dev->dispatch_state;
        info.attach_attempts = dev->attach_attempts;
        info.property_count = (dev->property_count < HWINFO_PROP_MAX) ? dev->property_count : HWINFO_PROP_MAX;

        for (uint32_t p = 0; p < info.property_count; p++) {
            const fabric_property_t* src = &dev->properties[p];
            strncpy(info.properties[p].key, src->key ? src->key : "", sizeof(info.properties[p].key) - 1);
            info.properties[p].type = src->type;
            if (src->type == FABRIC_PROP_U32) {
                info.properties[p].u32 = src->value.u32;
            } else if (src->type == FABRIC_PROP_STR) {
                strncpy(info.properties[p].str,
                        src->value.str ? src->value.str : "",
                        sizeof(info.properties[p].str) - 1);
            }
        }
        ...
        if (dev->bus_private && dev->name && strcmp(dev->name, "pci-device") == 0) {
            ...
            info.pci_primary_bus = pci->primary_bus;
            info.pci_secondary_bus = pci->secondary_bus;
            info.pci_subordinate_bus = pci->subordinate_bus;
            info.pci_interrupt_line = pci->interrupt_line;
            info.pci_interrupt_pin = pci->interrupt_pin;
            info.pci_cap_ptr = pci->cap_ptr;
            info.pci_cap_count = pci->cap_count;
            info.pci_command = pci->command;
            info.pci_status = pci->status;
            info.pci_capability_bits = pci->capability_bits;
