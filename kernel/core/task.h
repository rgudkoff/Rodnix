...
/* Полосы убийства при нехватке памяти — как jetsam: полоса на процесс,
 * убивают снизу вверх. Аудио выше переднего плана — не наша выдумка, а
 * порядок XNU (JETSAM_PRIORITY_AUDIO_AND_ACCESSORY > FOREGROUND). */
typedef enum {
    MEMBAND_BACKGROUND = 0,  /* фоновый рендер, batch: первый кандидат */
    MEMBAND_FOREGROUND = 1,  /* обычные процессы (умолчание) */
    MEMBAND_AUDIO      = 2,  /* аудио и его тракт */
    MEMBAND_CRITICAL   = 3,  /* не участвует в убийстве вовсе */
} mem_band_t;
...
typedef struct task {
    ...
    /* Полоса убийства при нехватке памяти (mem_band_t). Наследуется от
     * родителя при fork; убивают с нижней полосы вверх, внутри полосы — по
     * освобождаемому. CRITICAL и задачи ядра не участвуют. */
    uint8_t mem_band;
    /* Задача приговорена (принудительное завершение отправлено): циклы
     * нехватки в отказе обязаны прерваться и вернуть отказ — поток дойдёт
     * до смерти штатным путём, а не будет крутиться в ядре неубиваемым. */
    volatile uint8_t doomed;
...
typedef struct thread {
    ...
    /* Отказы страниц, взятые этим потоком. Растёт в vm_fault_handle().
     * Это будущий свидетель обещания реального времени (этап 7): у RT-потока
     * после предварительного отображения счётчик обязан стоять на месте. */
