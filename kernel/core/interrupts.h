...
/**
 * Reserve a free hardware interrupt vector in the given inclusive range.
 * Intended for APIC/MSI-style interrupts.
 * @return vector number on success, negative value on failure
 */
int interrupt_vector_alloc(uint32_t min_vector, uint32_t max_vector);

/**
 * Release a previously reserved interrupt vector.
 */
