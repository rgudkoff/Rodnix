...
static uint64_t paging_alloc_page_table_low(void)
{
    ...
    if (!phys) {
        /* Промежуточной таблице не нужен адрес ниже 16 МБ — ей нужен любой
         * кадр, достижимый через прямое отображение, которым эта же функция
         * его и зануляет. Ограничение LOW-зоной здесь исчерпывало таблицы
         * страниц раньше памяти: под давлением CT-044 зона в 16 МБ
         * кончалась при десятках тысяч свободных страниц NORMAL, отказ
         * возвращал E_GENERIC, и неразрешимый отказ init останавливал
         * процессор. Возврат памяти LOW-зону не пополняет — таблицы
         * умерших возвращаются только со смертью адресного пространства. */
        phys = pmm_alloc_page_in_zone(PMM_ZONE_NORMAL);
    }
    if (!phys) {
        return 0;
    }
