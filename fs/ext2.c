...
/* =========================================================================
 * Inode and directory entry allocation (write path)
 * ======================================================================= */

#define EXT2_FT_REG_FILE 1u
#define EXT2_FT_DIR      2u

/* Actual on-disk dirent size: header + name rounded to 4-byte boundary. */
static uint32_t ext2_dirent_actual_len(uint8_t name_len)
{
    uint32_t raw = (uint32_t)sizeof(ext2_dirent_hdr_t) + (uint32_t)name_len;
    return (raw + 3u) & ~3u;
}

/* Allocate a free inode from the inode bitmaps.  Caller holds g_ext2_rw_lock. */
static int ext2_alloc_inode(ext2_mount_ctx_t* ctx, int is_dir, uint32_t* out_ino_num)
{
    if (!ctx || !ctx->gdt || !out_ino_num || ctx->block_size == 0) {
        return RDNX_E_INVALID;
    }
    ...
    uint32_t first_ino = (ctx->sb.rev_level >= 1u) ? ctx->sb.first_ino : 11u;
    if (first_ino < 2u) {
        first_ino = 11u;
    }
    ...
    for (uint32_t g = 0; g < ctx->group_count; g++) {
        if (ctx->gdt[g].free_inodes_count == 0 || ctx->gdt[g].inode_bitmap == 0) {
            continue;
        }

        int rc = ext2_read_block(ctx, ctx->gdt[g].inode_bitmap, bmap);
        if (rc != RDNX_OK) {
            kfree(bmap);
            return rc;
        }

        uint32_t gbase_ino   = g * ctx->sb.inodes_per_group + 1u; /* 1-based */
        uint32_t group_inodes = ctx->sb.inodes_per_group;
        if (gbase_ino + group_inodes - 1u > ctx->sb.inodes_count) {
            group_inodes = ctx->sb.inodes_count - gbase_ino + 1u;
        }

        for (uint32_t bi = 0; bi < group_inodes; bi++) {
            uint32_t ino_num = gbase_ino + bi;
            if (ino_num < first_ino) {
                continue;
            }

            uint32_t byte = bi >> 3;
            uint8_t  mask = (uint8_t)(1u << (bi & 7u));
            if ((bmap[byte] & mask) != 0u) {
                continue;
            }

            bmap[byte] |= mask;
            rc = ext2_write_block(ctx, ctx->gdt[g].inode_bitmap, bmap);
            if (rc != RDNX_OK) {
                kfree(bmap);
                return rc;
            }

            if (ctx->gdt[g].free_inodes_count > 0u) {
                ctx->gdt[g].free_inodes_count--;
            }
            if (ctx->sb.free_inodes_count > 0u) {
                ctx->sb.free_inodes_count--;
            }
            if (is_dir && ctx->gdt[g].used_dirs_count != UINT16_MAX) {
                ctx->gdt[g].used_dirs_count++;
            }
...
/* Free an inode in the inode bitmap.  Caller holds g_ext2_rw_lock. */
static int ext2_free_inode(ext2_mount_ctx_t* ctx, uint32_t ino_num, int is_dir)
{
    if (!ctx || !ctx->gdt || ino_num == 0 || ino_num > ctx->sb.inodes_count) {
        return RDNX_E_INVALID;
    }

    uint32_t g  = (ino_num - 1u) / ctx->sb.inodes_per_group;
    uint32_t bi = (ino_num - 1u) % ctx->sb.inodes_per_group;
    if (g >= ctx->group_count || ctx->gdt[g].inode_bitmap == 0) {
        return RDNX_E_INVALID;
    }
    ...
    if (ctx->gdt[g].free_inodes_count != UINT16_MAX) {
        ctx->gdt[g].free_inodes_count++;
    }
    if (ctx->sb.free_inodes_count != UINT32_MAX) {
        ctx->sb.free_inodes_count++;
    }
    if (is_dir && ctx->gdt[g].used_dirs_count > 0u) {
        ctx->gdt[g].used_dirs_count--;
    }
...
/*
 * Add a directory entry into dir_ino_num.  dir_ino is updated if a new
 * block is allocated.  Caller holds g_ext2_rw_lock.
 */
static int ext2_dir_add_entry(ext2_mount_ctx_t* ctx,
                               uint32_t dir_ino_num,
                               ext2_inode_t* dir_ino,
                               uint32_t child_ino_num,
                               const char* name,
                               uint8_t file_type)
{
    if (!ctx || !dir_ino || !name || child_ino_num == 0) {
        return RDNX_E_INVALID;
    }

    uint8_t name_len = 0;
    while (name[name_len] && name_len < 255u) {
        name_len++;
    }
    if (name_len == 0) {
        return RDNX_E_INVALID;
    ...
    uint32_t needed = ext2_dirent_actual_len(name_len);
    if (needed > ctx->block_size) {
        return RDNX_E_INVALID;
    }
    ...
    while ((uint64_t)lbn * ctx->block_size < dir_size) {
        uint32_t pblk = 0;
        int rc = ext2_inode_get_block(ctx, dir_ino, lbn, &pblk);
        if (rc != RDNX_OK || pblk == 0) {
            lbn++;
        ...
        uint32_t pos = 0;
        while (pos + sizeof(ext2_dirent_hdr_t) <= ctx->block_size) {
            ext2_dirent_hdr_t* de = (ext2_dirent_hdr_t*)&blk[pos];
            if ((uint32_t)de->rec_len < sizeof(ext2_dirent_hdr_t) ||
                pos + (uint32_t)de->rec_len > ctx->block_size) {
                break;
            }

            if (de->inode == 0u) {
                /* Deleted / free slot: reuse if big enough. */
                if ((uint32_t)de->rec_len >= needed) {
                    de->inode     = child_ino_num;
                    de->name_len  = name_len;
                    de->file_type = file_type;
                    memcpy(&blk[pos + sizeof(ext2_dirent_hdr_t)], name, name_len);
                    rc = ext2_write_block(ctx, pblk, blk);
                    kfree(blk);
                    return rc;
                }
            } else {
                uint32_t actual = ext2_dirent_actual_len(de->name_len);
                uint32_t slack  = (uint32_t)de->rec_len - actual;
                if (slack >= needed) {
                    uint16_t old_rec = de->rec_len;
                    de->rec_len = (uint16_t)actual;
                    ext2_dirent_hdr_t* ne = (ext2_dirent_hdr_t*)&blk[pos + actual];
                    ne->inode     = child_ino_num;
                    ne->rec_len   = (uint16_t)(old_rec - (uint16_t)actual);
                    ne->name_len  = name_len;
                    ne->file_type = file_type;
                    memcpy(&blk[pos + actual + sizeof(ext2_dirent_hdr_t)], name, name_len);
                    rc = ext2_write_block(ctx, pblk, blk);
                    kfree(blk);
                    return rc;
                }
    ...
    /* No gap found — allocate a new directory block. */
    int inode_dirty = 0;
    uint32_t pblk = 0;
    int rc = ext2_inode_get_or_alloc_block(ctx, dir_ino_num, dir_ino,
                                            lbn, 1, &pblk, &inode_dirty);
    if (rc != RDNX_OK || pblk == 0) {
        kfree(blk);
        return (rc != RDNX_OK) ? rc : RDNX_E_GENERIC;
    }

    memset(blk, 0, ctx->block_size);
    ext2_dirent_hdr_t* de = (ext2_dirent_hdr_t*)blk;
    de->inode     = child_ino_num;
    de->rec_len   = (uint16_t)ctx->block_size;
    de->name_len  = name_len;
    de->file_type = file_type;
    memcpy(blk + sizeof(ext2_dirent_hdr_t), name, name_len);
    ...
    uint64_t new_dir_size = ((uint64_t)lbn + 1u) * ctx->block_size;
    dir_ino->size_lo   = (uint32_t)(new_dir_size & 0xFFFFFFFFu);
    dir_ino->size_high = 0u;
    rc = ext2_write_inode(ctx, dir_ino_num, dir_ino);
    kfree(blk);
    return rc;
}

/*
 * Remove a named entry from a directory block by name.
 * Merges the removed slot into the preceding entry (or marks inode=0 for the
 * first slot).  Caller holds g_ext2_rw_lock.
 */
static int ext2_dir_remove_entry(ext2_mount_ctx_t* ctx,
                                  uint32_t dir_ino_num,
                                  const ext2_inode_t* dir_ino,
                                  const char* name)
{
    ...
    while ((uint64_t)lbn * ctx->block_size < dir_size) {
        ...
        while (pos + sizeof(ext2_dirent_hdr_t) <= ctx->block_size) {
            ...
            if (de->inode != 0u && de->name_len == name_len) {
                const char* dname =
                    (const char*)&blk[pos + sizeof(ext2_dirent_hdr_t)];
                int match = 1;
                ...
                if (match) {
                    if (has_prev) {
                        ext2_dirent_hdr_t* pd = (ext2_dirent_hdr_t*)&blk[prev_pos];
                        pd->rec_len = (uint16_t)((uint32_t)pd->rec_len +
                                                 (uint32_t)de->rec_len);
                    } else {
                        de->inode = 0u;
                    }
...
/* Free all data blocks of an inode then free the inode itself.
 * Caller holds g_ext2_rw_lock. */
static int ext2_free_inode_and_blocks(ext2_mount_ctx_t* ctx,
                                       uint32_t ino_num,
                                       ext2_inode_t* ino,
                                       int is_dir)
{
    int rc = ext2_trim_inode_blocks(ctx, ino_num, ino, 0u);
    if (rc != RDNX_OK) {
        return rc;
    }
    return ext2_free_inode(ctx, ino_num, is_dir);
}

/* =========================================================================
 * Exported write-path entry points
 * ======================================================================= */
...
int ext2_create_file(vfs_node_t* parent_node, const char* name,
                     ...
                     vfs_node_t** out_node)
{
    if (!parent_node || !parent_node->inode || !name || !out_node) {
        return RDNX_E_INVALID;
    }
    ...
    uint32_t parent_ino_num = (uint32_t)parent_node->inode->fs_ino;
    ...
    new_ino.mode        = (uint16_t)(EXT2_S_IFREG | (mode & 0x0FFFu));
    new_ino.uid         = (uint16_t)(uid & 0xFFFFu);
    new_ino.gid         = (uint16_t)(gid & 0xFFFFu);
    new_ino.links_count = 1u;
    ...
    rc = ext2_dir_add_entry(&g_ext2_live, parent_ino_num, &parent_ino,
    ...
    ext2_mark_node(node, new_ino_num, &new_ino);
                    ...
                    vfs_node_t** out_node)
{
    ...
    vfs_node_t* node = vfs_fs_alloc_node(name, VFS_NODE_DIR);
    if (!node) {
        return RDNX_E_NOMEM;
    }
    ...
    /* Allocate the first block for "." / ".." entries. */
    uint32_t data_blk = 0;
    ...
    uint32_t dot_len = ext2_dirent_actual_len(1u);

    /* "." entry */
    ext2_dirent_hdr_t* dot = (ext2_dirent_hdr_t*)blk;
    dot->inode     = new_ino_num;
    dot->rec_len   = (uint16_t)dot_len;
    dot->name_len  = 1u;
    dot->file_type = EXT2_FT_DIR;
    blk[sizeof(ext2_dirent_hdr_t)] = '.';

    /* ".." entry — spans the rest of the block */
    ext2_dirent_hdr_t* dotdot = (ext2_dirent_hdr_t*)&blk[dot_len];
    dotdot->inode     = parent_ino_num;
    dotdot->rec_len   = (uint16_t)(g_ext2_live.block_size - dot_len);
    dotdot->name_len  = 2u;
    dotdot->file_type = EXT2_FT_DIR;
    blk[dot_len + sizeof(ext2_dirent_hdr_t)]     = '.';
    blk[dot_len + sizeof(ext2_dirent_hdr_t) + 1] = '.';

    rc = ext2_write_block(&g_ext2_live, data_blk, blk);
    kfree(blk);
    if (rc != RDNX_OK) {
        (void)ext2_free_block(&g_ext2_live, data_blk);
        (void)ext2_free_inode(&g_ext2_live, new_ino_num, 1);
    ...
    ext2_inode_t new_ino;
    memset(&new_ino, 0, sizeof(new_ino));
    new_ino.mode        = (uint16_t)(EXT2_S_IFDIR | (mode & 0x0FFFu));
    new_ino.uid         = (uint16_t)(uid & 0xFFFFu);
    new_ino.gid         = (uint16_t)(gid & 0xFFFFu);
    new_ino.links_count = 2u; /* parent dir entry + "." */
    new_ino.size_lo     = g_ext2_live.block_size;
    new_ino.block[0]    = data_blk;
    new_ino.blocks      = sectors_per_block;
    ...
    /* Parent gains a link_count for the ".." back-reference. */
    if (parent_ino.links_count != UINT16_MAX) {
        parent_ino.links_count++;
        (void)ext2_write_inode(&g_ext2_live, parent_ino_num, &parent_ino);
    }

    rc = ext2_dir_add_entry(&g_ext2_live, parent_ino_num, &parent_ino,
                             new_ino_num, name, EXT2_FT_DIR);
    if (rc != RDNX_OK) {
...
{
    if (!node || !node->inode || !node->parent || !node->parent->inode) {
        return RDNX_E_INVALID;
    }
    ...
    uint32_t parent_num = (uint32_t)node->parent->inode->fs_ino;
    ...
    if (ino.links_count > 0u) {
        ino.links_count--;
    }
    if (ino.links_count == 0u) {
        rc = ext2_free_inode_and_blocks(&g_ext2_live, ino_num, &ino, is_dir);
    } else {
        rc = ext2_write_inode(&g_ext2_live, ino_num, &ino);
    }

    /* Directory ".." loses the back-link from the parent. */
    if (is_dir && parent_ino.links_count > 0u) {
        parent_ino.links_count--;
