...
static inline long posix_listen(int fd, int backlog)
{
    return rdnx_syscall2(POSIX_SYS_LISTEN, (long)fd, (long)backlog);
}

/* accept: a3 = timeout_ms (0 = block forever) */
static inline long posix_accept(int fd, void* addr, uint64_t timeout_ms)
{
    return rdnx_syscall3(POSIX_SYS_ACCEPT, (long)fd,
                         (long)(uintptr_t)addr, (long)timeout_ms);
}
